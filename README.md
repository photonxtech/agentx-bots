...
│   └── README.md      (mirrors this file)
└── docs/              Original design spec and implementation plan (historical reference)
```
...
Then edit `Ai/backend/.env` and set at minimum:
...
```bash
./Ai/backend/run.sh     # http://localhost:8001  (terminal 1)
./Ai/frontend/run.sh    # http://localhost:5173   (terminal 2)
```
...
- If port `5173` is already taken by something else on your machine, Vite silently picks the
  ...
  sure that origin is listed in `Ai/backend/.env`'s `CORS_ORIGINS` (both are included by default).
...
1. **Spelling correction** (`Ai/backend/app/llm/query_normalizer.py`) — fuzzy-matches typos
   against the site's own crawled vocabulary first (catches things a generic dictionary misses,
   e.g. brand names), falls back to `pyspellchecker`, and detects/merges accidentally split words
   ("plan form" → "platform"). Only affects retrieval/generation input, not what's stored in
...
4. **Cross-encoder reranking** (`Ai/backend/app/llm/reranker.py`) — scores each (question, chunk)
   pair directly, against **both** the bare question and a version with the website's name folded
   in ("PhotonX: how to contact"), keeping whichever scoring is higher per chunk — some
   short/vague questions need that site-name context to score well, others score dramatically
   *worse* with it, so neither framing alone is reliable.
...
## Configuration reference (`Ai/backend/.env`)
...
- `Ai/backend/run.sh` intentionally does **not** use `uvicorn --reload`. ChromaDB's embedded,
...
- ChromaDB's HNSW `ef_search` defaults to 10, which is lower than the number of neighbors this
  ...
  `Ai/backend/app/vectordb/collection.py` by setting `ef_search=200` explicitly; if you change
  `TOP_K_CHUNKS` to something much larger, raise this too.
- `Ai/backend/requirements.txt` intentionally does **not** include bare
  `langchain`/`langchain-community` — only `langchain-text-splitters` and `langchain-huggingface`
  are actually imported anywhere.

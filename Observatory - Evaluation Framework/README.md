...
configurations, with real DeepEval metrics and LangSmith tracing.

| | |
|---|---|
| **Deploy to AWS** | [DEPLOY.md](DEPLOY.md) — Fargate + RDS + EFS via CDK |
| **Run locally** | `cp .env.example .env`, add `GROQ_API_KEY`, then `docker compose up --build` |
| **Configuration** | [.env.example](.env.example) documents every variable |
| **Infrastructure** | [infra/](infra/) — AWS CDK (Python) |
| **Health** | `GET /healthz` liveness, `GET /readyz` dependency readiness |

> **Secrets never live in this repository.** `.env` is git-ignored; on AWS the
> keys come from Secrets Manager. This repo is public — treat any key that lands
> here as compromised.

...
def _build_metrics(judge):
    ...
    common = dict(
        ...
        # DeepEval's async path drives its own event loop; we stay serial for
        # Groq's rate limit and to keep clear of FastAPI's loop.
        async_mode=False,
    )
...
def _run_sync(results: list[dict], notes: list[str], judge_model: str | None = None) -> dict:
    ...
    for result in results:
        per_sample.append(_score_one(result, judge, notes))
...
async def evaluate_with_deepeval(
    ...
    if not groq_ready():
        notes.append("No Groq API key configured — DeepEval metrics were not computed.")

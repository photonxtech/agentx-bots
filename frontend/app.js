...
async function bootSplash() {
    ...
    const MIN_DWELL = 1000;              // comfortable dwell for wordmark
    const MAX_DWELL = 2500;              // hard cap: splash MUST disappear by 2.5s
    ...
    const dismiss = () => {
        ...
        try {
            ...
            console.error('Error staging draft wizard:', e);
        }
        if (!splash) return;
    ...
    // Fail-safe hard timeout: UI never hangs even if network or server is slow
    const hardTimeout = setTimeout(dismiss, MAX_DWELL);
...
function openDraftWizard() {
    ...
    renderSeedQAPreview(null);   // clear any leftover seed QA display
...
function bindEvents() {
    ...
    // ── Seed Q&A upload ───────────────────────────────────────────────────
    const seedDropArea  = document.getElementById('seedQADropArea');
    const seedFileInput = document.getElementById('seedQAFileInput');

    seedDropArea.addEventListener('click', () => seedFileInput.click());
    seedDropArea.addEventListener('dragover', e => { e.preventDefault(); seedDropArea.classList.add('dragover'); });
    seedDropArea.addEventListener('dragleave', () => seedDropArea.classList.remove('dragover'));
    seedDropArea.addEventListener('drop', e => {
        e.preventDefault();
        seedDropArea.classList.remove('dragover');
        if (e.dataTransfer.files[0]) handleSeedQAUpload(e.dataTransfer.files[0]);
    });
    seedFileInput.addEventListener('change', e => {
        if (e.target.files[0]) handleSeedQAUpload(e.target.files[0]);
        e.target.value = '';
    });

    document.getElementById('btnRemoveSeedQA').addEventListener('click', removeSeedQA);
}
...
async function loadSessions() {
    try {
        ...
        const res = await fetch(`${API}/api/sessions`, { signal: controller.signal });
        clearTimeout(timer);
        if (res.ok) {
...
function renderSessionWorkspace() {
    ...
    // Restore seed Q&A preview if present (persists across re-generation).
    if (data.seed_qa_json && data.seed_qa_json.length > 0) {
        renderSeedQAPreview(data.seed_qa_json);
    } else {
        renderSeedQAPreview(null);
    }

    if (data.qa_json && data.qa_json.length > 0) {
        // QA already generated — show combined preview (seed + generated)
        const seedPairs = data.seed_qa_json || [];
        renderQAPreview(data.qa_json, undefined, data.qa_meta, seedPairs);
    ...
    } else if (data.seed_qa_json && data.seed_qa_json.length > 0) {
        // Only seed Q&A — no generated Q&A yet. Show seed preview and land on
        // stage 2 so the user can still generate (or skip straight to eval).
...
// ═══════════════════════════════════════
// SEED Q&A — upload / remove / preview
// ═══════════════════════════════════════

/** Upload a user-provided Q&A file (JSON or CSV) to the session as seed pairs. */
async function handleSeedQAUpload(file) {
    if (!file) return;

    // A draft flow has no session yet — trigger the same lazy-creation used by
    // document upload so the user's seed pairs are never lost.
    try {
        await ensureSession();
    } catch (err) {
        showToast(err.message, 'error');
    ...
    try {
        showToast(`Uploading seed Q&A from ${file.name}…`, 'info');
        const res = await fetch(`${API}/api/sessions/${activeSessionId}/upload-seed-qa`, {
            method: 'POST',
            body: formData,
        });
        if (!res.ok) {
        ...
        // Fetch the full seed Q&A from the session to keep local state in sync.
        const sessionRes = await fetch(`${API}/api/sessions/${activeSessionId}`);
        if (sessionRes.ok) {
            const sessionData = await sessionRes.json();
            if (activeSessionData) activeSessionData.seed_qa_json = sessionData.seed_qa_json;
            renderSeedQAPreview(sessionData.seed_qa_json);
        }
        ...
        const idx = sessions.findIndex(s => s.id === activeSessionId);
        if (idx !== -1) sessions[idx].has_seed_qa = true;
        renderSessionList();

        showToast(`${result.total} seed Q&A pair(s) uploaded`, 'success');
    } catch (err) {
        showToast('Seed Q&A upload failed: ' + err.message, 'error');
    }
}


/** Remove all uploaded seed Q&A pairs from the current session. */
async function removeSeedQA() {
    if (!activeSessionId) return;
    if (!confirm('Remove all uploaded Q&A pairs? This cannot be undone.')) return;
    try {
        const res = await fetch(`${API}/api/sessions/${activeSessionId}/seed-qa`, { method: 'DELETE' });
        if (!res.ok) {
            const err = await res.json().catch(() => ({}));
            throw new Error(err.detail || `Failed (${res.status})`);
        }
        if (activeSessionData) activeSessionData.seed_qa_json = null;
        renderSeedQAPreview(null);
        refreshStageProgress();

        const idx = sessions.findIndex(s => s.id === activeSessionId);
        if (idx !== -1) sessions[idx].has_seed_qa = false;
        renderSessionList();

        showToast('Seed Q&A removed', 'info');
    } catch (err) {
        showToast(err.message, 'error');
    }
}
...
/** Render (or clear) the seed Q&A preview table in Stage 1.
 *  seedPairs = null/[] to hide the preview. */
function renderSeedQAPreview(seedPairs) {
    const previewEl = document.getElementById('seedQAPreview');
    const tbody     = document.getElementById('seedQAPreviewBody');
    const countEl   = document.getElementById('seedQACount');

    if (!previewEl || !tbody || !countEl) return;

    const pairs = seedPairs && seedPairs.length ? seedPairs : null;
    if (!pairs) {
    ...
    countEl.textContent = `${pairs.length} Q&A pair${pairs.length === 1 ? '' : 's'} uploaded`;
    tbody.innerHTML = pairs.map((p, i) => `
        <tr>
            <td>${i + 1}</td>
            <td>${escapeHtml(p.question || '')}</td>
            <td>${escapeHtml(p.answer || '')}</td>
        </tr>
    `).join('');
...
async function generateQA() {
    ...
    try {
        ...
        // The server returns the COMBINED preview (seed + generated).
        // Locally we split them by source tag for rendering.
        // data.meta carries seed_count and generated_count.
        const seedPairsFromMeta = (activeSessionData && activeSessionData.seed_qa_json) || [];
        const seedCount   = data.meta && data.meta.seed_count   !== undefined ? data.meta.seed_count   : seedPairsFromMeta.length;
        const genCount    = data.meta && data.meta.generated_count !== undefined ? data.meta.generated_count : (data.preview.length - seedCount);
        const genPreview  = data.preview.slice(seedCount);  // generated ones come after seed

        // Update local state — only the generated portion lives in qa_json.
        if (activeSessionData) activeSessionData.qa_json = genPreview;

        // Render combined preview distinguishing seed from generated.
        renderQAPreview(genPreview, genCount, data.meta, seedPairsFromMeta);
        ...
        const totalMsg = seedCount
            ? `${genCount} new Q&A generated · ${data.total} total (${seedCount} from your upload + ${genCount} new)`
            : `${data.total} QA test cases generated`;
        showToast(totalMsg, 'success');
    } catch (err) {
        showToast(err.message, 'error');
    } finally {
...
/**
 * Render the QA preview table for GENERATED pairs.
 * If seedPairs is supplied, render a separate seed section above the generated table.
 *
 * @param {Array}  qaPairs    - system-generated Q&A pairs
 * @param {number} [total]    - total count label
 * @param {Object} [meta]     - generation metadata
 * @param {Array}  [seedPairs]- user-uploaded seed Q&A pairs (optional)
 */
function renderQAPreview(qaPairs, total, meta, seedPairs) {
    const tbody = document.getElementById('qaPreviewBody');
    tbody.innerHTML = qaPairs.map((qa, i) => `
        <tr>
            <td>${i + 1}</td>
            <td>${escapeHtml(qa.question || qa.question)}</td>
            <td>${escapeHtml(qa.answer || qa.answer)}</td>
        </tr>
    `).join('');

    const seedCount = (seedPairs || []).length;
    const genCount  = qaPairs.length;
    const countEl   = document.getElementById('qaCount');
    if (seedCount) {
        countEl.innerHTML = `<span class="qa-count-gen">${genCount} generated</span>` +
            ` <span class="qa-count-plus">+</span>` +
            ` <span class="qa-count-seed">${seedCount} from your upload</span>` +
            ` = <strong>${genCount + seedCount} total</strong>`;
    } else {
        countEl.textContent = `${genCount} test cases generated`;
    }
    ...
    const notes = (meta.notes || []).length
        ...
        : '';
    const dupNote = meta.duplicates_filtered
        ? `<div class="qa-prov-dedup">🔍 ${meta.duplicates_filtered} duplicate(s) filtered — questions matching your seed upload were removed.</div>`
        : '';
    ...
    banner.innerHTML = `
        ...
        <div class="qa-prov-meta">
            ...
            ${seedCount ? `· <strong>${seedCount}</strong> from your upload` : ''}
        </div>
        ${dupNote}
        ${notes}`;
}
...
function appendRunCard(runConfig, container) {
    ...
    const resultsHtml = (runConfig.results || []).map((r, i) => {
            const src = (r.metrics && r.metrics._source) || 'generated';
            const srcBadge = src === 'seed'
                ? `<span class="result-src-badge seed" title="From your uploaded Q&A">Uploaded</span>`
                : `<span class="result-src-badge generated" title="System-generated">Generated</span>`;
            return `
            <tr>
                <td>${i + 1}</td>
                <td>${srcBadge}</td>
...
function stageState() {
    ...
    // Stage 2 (test set) is considered done when EITHER the system generated
    // Q&A OR the user uploaded their own seed Q&A. Either alone is sufficient
    // to unlock the configuration stage.
    const hasQA = !!(d.qa_json && d.qa_json.length) || !!(d.seed_qa_json && d.seed_qa_json.length);
    return {
        1: docs > 0,
        2: hasQA,
        3: runs > 0,
...
function renderReviewSummary() {
    ...
    const genCases  = (d.qa_json || []).length;
    const seedCases = (d.seed_qa_json || []).length;
    const totalCases = genCases + seedCases;
    const runs = (d.run_configs || []).length;
    ...
    let testSetLabel;
    if (!totalCases) {
        testSetLabel = '<span class="rs-none">none</span>';
    } else if (seedCases && genCases) {
        testSetLabel = `${totalCases} cases (${genCases} generated + ${seedCases} from your upload)`;
    } else if (seedCases) {
        testSetLabel = `${seedCases} case${seedCases === 1 ? '' : 's'} (from your upload)`;
    } else {
        testSetLabel = `${genCases} golden case${genCases === 1 ? '' : 's'}`;
    }

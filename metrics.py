...
- Output exactly ONE integer for EACH context chunk.
- If there is 1 chunk, output exactly 1 integer.
- If there are 3 chunks, output exactly 3 integers.
- Do not invent extra chunks.
- Do not omit chunks.
- Each value must be either 1 or 0.
- 1 = relevant for answering the question.
- 0 = not relevant.
- Return JSON only.
"""

def context_precision(question, contexts):
    print("Entered context_precision()", flush=True)
    print(f"Contexts received: {len(contexts)}", flush=True)
    if not contexts:
        return None
    try:
        ...
        print("Raw judge response:", resp, flush=True)
        ...
        if len(verdicts) != len(contexts):
            print(
                f"Expected {len(contexts)} verdicts but got {len(verdicts)}",
                    flush=True,
            )
            return None
...
def evaluate(question, answer, contexts):
    ...
    print("evaluate() called", flush=True)
    print(f"Question: {question}", flush=True)
    print(f"Number of contexts: {len(contexts)}", flush=True)

    context_blob = "\n\n".join(contexts)

    with ThreadPoolExecutor(max_workers=4) as pool:
        print("Submitting metrics...", flush=True)
        ...
        print("Faithfulness:", faith, flush=True)

        rel = rel_f.result()
        print("Answer relevancy:", rel, flush=True)

        prec = prec_f.result()
        print("Context precision:", prec, flush=True)

        ctxrel = ctxrel_f.result()
        print("Context relevancy:", ctxrel, flush=True)

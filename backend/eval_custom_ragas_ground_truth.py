...
CSV_PATH = Path(__file__).parent / "ground_truth_template.csv"
...
def main():
    if not CSV_PATH.exists():
        raise SystemExit(
            f"Couldn't find {CSV_PATH}. Export your filled-in ground truth "
            f"sheet to CSV (same filename) into this folder first."
        )
    df = pd.read_csv(CSV_PATH)
    labeled = df[df["ground_truth"].notna() & (df["ground_truth"].astype(str).str.strip() != "")]
    print(f"Found {len(labeled)} labeled rows out of {len(df)} total.\n")

    results = []
    for i, row in labeled.reset_index(drop=True).iterrows():
        contexts = json.loads(row["contexts_json"])
        gt = str(row["ground_truth"])
        answer = str(row["current_answer"])
        recall = context_recall(gt, contexts)
        ...
        print(f"[{i+1}/{len(labeled)}] recall={_round(recall)} correctness={_round(correctness)}")

...
def _resolve_models(config, notes: list[str]) -> tuple[str, str]:
    ...
    # If the user selected moonshotai/kimi-k3 or Nvidia model and Nvidia is ready
    if (requested == "moonshotai/kimi-k3" or requested == nvidia_model) and _nvidia_ready():
        generator = requested
        notes.append(f"Answer generation using Nvidia NIM model '{generator}'")
    # If the user selected a valid Groq model and Groq is ready, use it
    elif requested in GROQ_CHAT_MODELS and _groq_ready():
        generator = requested
    elif requested == OPENROUTER_model and OPENROUTER_key and OPENROUTER_key.startswith("sk-"):
        generator = requested
        notes.append(f"Answer generation using OpenRouter model '{generator}'")
    elif _nvidia_ready() and (not requested or requested == "moonshotai/kimi-k3"):
        generator = nvidia_model
        notes.append(f"Using Nvidia NIM model '{generator}' for answer generation")
    elif _groq_ready():
    ...
    elif OPENROUTER_key and OPENROUTER_key.startswith("sk-"):
        generator = OPENROUTER_model
        notes.append(f"Groq unavailable — using OpenRouter model '{generator}' for answer generation")
    else:
        generator = requested or GROQ_MODEL
...
async def _generate_answer(question: str, contexts: list[str], config, model: str) -> str:
    ...
    chat model (Groq, gateway, or OpenRouter) at the configured temperature."""
...
Answer:"""
    ...
    # Nvidia NIM routing (if requested model matches Nvidia or Nvidia is ready)
    nvidia_slots = []
    ...
    nvidia_models = {s["model"] for s in nvidia_slots}
    if (model in nvidia_models or (model and (model.startswith("moonshotai/") or model.startswith("deepseek/")))) and _nvidia_ready():
        for slot in nvidia_slots:
            target_model = model if model in nvidia_models else slot["model"]
            try:
                ...
                logger.warning("Nvidia NIM slot (%s) answer generation failed (%s), trying next...", slot["model"], e)

    OPENROUTER_key = os.getenv("OPENROUTER_API_KEY", "").strip()
    OPENROUTER_key_2 = os.getenv("OPENROUTER_API_KEY_2", "").strip()
    OPENROUTER_model = os.getenv("OPENROUTER_MODEL", "nvidia/nemotron-3.5-lightning:free").strip()

    # OpenRouter routing (if requested model is on OpenRouter or Groq is unavailable)
    if (model == OPENROUTER_model or not _groq_ready()) and OPENROUTER_key and OPENROUTER_key.startswith("sk-"):
        target_model = model if ("/" in model and not model.startswith("openai/gpt-oss")) else OPENROUTER_model
        for k in [OPENROUTER_key, OPENROUTER_key_2]:
            if not k:
                continue
            try:
                ...
                logger.warning("OpenRouter attempt failed (%s), trying fallback...", e)

    # Groq / Gateway routing
    if not _groq_ready():
        return "[No LLM API key configured — cannot generate an answer]"

    try:
        ep = await resolve_endpoint()
        groq_model = model if model in GROQ_CHAT_MODELS else (GROQ_MODEL if GROQ_MODEL in GROQ_CHAT_MODELS else "openai/gpt-oss-120b")
...
async def run_rag_pipeline(
    ...
    async def _process_one(qa: dict) -> dict:
    ...
    # Run answer generation concurrently across all QA pairs
    results = await asyncio.gather(*[_process_one(qa) for qa in qa_pairs])
    return list(results)

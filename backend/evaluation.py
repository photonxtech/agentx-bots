...
# Import the local embedding client from our RAG engine
from rag_engine import LocalEmbeddingClient

logger = logging.getLogger(__name__)

# Configuration constants (replacing config.py properties)
RAGAS_EVAL_MODEL = os.getenv("GROQ_EVALUATION_MODEL", "llama-3.1-8b-instant")
RAGAS_STRICT_FALLBACK_MODEL = "llama-3.1-8b-instant"
RAGAS_MAX_RETRIES = 3
RAGAS_TIMEOUT_S = 30.0
RAGAS_MAX_RETRY_WAIT = 60.0
RAGAS_MAX_TOKENS_CAP = 4096
RAGAS_RELEVANCY_N = 3
...
def _client() -> Groq:
    ...
    if not key or key.startswith("gsk_your"):
        raise RuntimeError(
            "No Groq API key set for RAGAS evaluation. Add EVALUATION_API_KEY "
...
def _judge_json(system: str, user: str, max_tokens: int = 500) -> dict | None:
    ...
    for attempt in range(RAGAS_MAX_RETRIES + 1):
        try:
            ...
            if (
                is_rate_limit
                and attempt < RAGAS_MAX_RETRIES
                and wait is not None
                and wait <= RAGAS_MAX_RETRY_WAIT
            ):
                time.sleep(wait + 0.5)
                continue
            if (
                is_token_limit
                and attempt < RAGAS_MAX_RETRIES
                and current_max_tokens < RAGAS_MAX_TOKENS_CAP
            ):
...
def _judge_verdicts(system: str, user: str, count: int, max_tokens: int = 500) -> list[int] | None:
    ...
    Always uses Groq structured JSON schema output (on llama-3.3-70b-versatile)
    directly on the first attempt to prevent verdict miscounts and retries.
    """
...
def _judge_verdicts_strict(system: str, user: str, count: int, max_tokens: int) -> list[int] | None:
    """Force exactly `count` 0/1 verdicts via Groq's json_object mode,
    with robust verification of the output count."""
    try:
        strict_user = (
            f"{user}\n\n"
            f"IMPORTANT: You MUST return a JSON object with exactly one key named \"verdicts\". "
            f"The value of \"verdicts\" MUST be a JSON array containing exactly {count} integers (either 0 or 1), "
            f"corresponding to the {count} items in the input, in the exact same order. "
            f"Do not return any other keys."
        )
        resp = _client().chat.completions.create(
            model=RAGAS_STRICT_FALLBACK_MODEL,
            temperature=0.0,
        ...
        logger.warning("RAGAS strict fallback still returned the wrong verdict count: %r", parsed)
        return None
    except Exception:
        logger.warning("RAGAS strict fallback call failed", exc_info=True)
        return None
...
def answer_relevancy(question: str, answer: str, n: int | None = None) -> float | None:
    ...
    try:
        ...
        # Resolve embeddings client locally
        embedding_client = LocalEmbeddingClient()
        vecs = embedding_client.get_embeddings_batch([question] + gen_qs)  # (1 + m, dim), normalized
        q_vec = np.array(vecs[0])
        gen_vecs = np.array(vecs[1:])
...
# --------------------------------------------------------------------------- #
_CTXREV_SYS = (
    "You analyze a CONTEXT passage and count sentences. "
    "Count the total number of sentences in the context, and count how many of those "
    "sentences are directly relevant to answering the question. "
    "Respond with a JSON object of the form:\n"
    '{"total_sentences": <int>, "relevant_sentences": <int>}\n'
...
def context_relevancy(question: str, contexts: list[str]) -> float | None:
    ...
    how many does the judge consider relevant vs noise/padding? Score =
    (relevant sentence count) / (total sentence count). Returns None (logged) if
    the judge/JSON step fails.
    """
    if not contexts:
        return None
    context_blob = "\n\n".join(contexts)
    try:
        resp = _judge_json(
            _CTXREV_SYS, f"QUESTION: {question}\n\nCONTEXT:\n{context_blob}", max_tokens=100
        )
        if resp is None:
            return None
        total = resp.get("total_sentences")
        relevant = resp.get("relevant_sentences")
        if total is None or relevant is None:
            logger.warning(
                "RAGAS context_relevancy: missing total_sentences/relevant_sentences: %r", resp
            )
            return None
        try:
            total, relevant = int(total), int(relevant)
        except (TypeError, ValueError):
            return None
        if total == 0:
            return 0.0
...
def answer_correctness(answer: str, ground_truth: str) -> float | None:
    ...
    try:
        ...
        # Resolve embeddings client locally
        embedding_client = LocalEmbeddingClient()
        vecs = embedding_client.get_embeddings_batch([answer, ground_truth])
        semantic_sim = float(np.clip(np.array(vecs[0]) @ np.array(vecs[1]), 0.0, 1.0))
